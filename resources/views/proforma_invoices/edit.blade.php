                        ...
                        <div name="proformaInvoiceFormGroup" class="columns is-marginless is-multiline">
                            <div class="column is-6">
                                <div class="field">
                                    ...
                                    <div class="control has-icons-left">
                                        <div class="select is-fullwidth">
                                            <select id="proformaInvoice[{{ $loop->index }}][product_id]" name="proformaInvoice[{{ $loop->index }}][product_id]" onchange="getProductSelected(this.id, this.value)">
                                                <option selected disabled>Select Product</option>
                                                @foreach ($products as $product)
                                                    <option value="{{ $product->id }}" {{ $proformaInvoiceDetail->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="icon is-small is-left">
