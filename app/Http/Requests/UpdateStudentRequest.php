...
class UpdateStudentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'route_id' => ['nullable', 'integer', new MustBelongToCompany('routes')],
            'vehicle_id' => ['nullable', 'integer', new MustBelongToCompany('vehicles')],
            'first_name' => ['required', 'string', 'max:15'],
            'last_name' => ['nullable', 'string', 'max:15'],
            'gender' => ['required', 'string', 'max:6', Rule::in(['male', 'female'])],
            'email' => ['nullable', 'string', 'email', 'max:30', Rule::unique('students')->ignore($this->route('student')->id)],
            'phone' => ['nullable', 'string', 'max:15', Rule::unique('students')->ignore($this->route('student')->id)],
