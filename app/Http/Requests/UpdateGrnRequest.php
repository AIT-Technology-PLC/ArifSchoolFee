...
class UpdateGrnRequest extends FormRequest
{
    ...
    {
        return [
            'code' => ['required', 'string', new UniqueReferenceNum('grns', $this->route('grn')->id), function ($attribute, $value, $fail) {
                if ($this->get('code') != nextReferenceNumber('grns') && !userCompany()->isEditingReferenceNumberEnabled()) {
