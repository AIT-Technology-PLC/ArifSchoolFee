...
class InventoryBatchReport
{
    ...
    private function setQuery()
    {
        $this->query = MerchandiseBatch::query()
            ...
            ->when(isset($this->filters['availability']) && $this->filters['availability'] == 'Out Of Stock', fn($q) => $q->where('merchandise_batches.quantity', '<', 0))
            ...
            ->when(isset($this->filters['expiry']) && $this->filters['expiry'] == 'Near Expiry', fn($q) => $q->nearToBeExpired())
