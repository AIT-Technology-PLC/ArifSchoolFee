...
class ReturnService
{
    ...
    public function add($return, $user)
    {
        ...
        DB::transaction(function () use ($return) {
            ...
            $this->updateReturnedQuantityOnGdnDetails($return);
        });

        return [true, ''];
    }

    public function approveAndAdd($return, $user)
    {
        if (!$user->hasWarehousePermission('add',
            $return->returnDetails->pluck('warehouse_id')->toArray())) {
            return [false, 'You do not have permission to add to one or more of the warehouses.'];
        ...
        DB::transaction(function () use ($return) {
            (new ApproveTransactionAction)->execute($return);
    ...
    private function updateReturnedQuantityOnGdnDetails($return)
    {
        if (!userCompany()->isReturnLimitedBySales()) {
            return;
        }
